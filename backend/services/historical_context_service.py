...
from src.parser.direction_classifier import classify_fund, is_other_direction  # noqa: E402
...
def _aggregate_today(records: List[TradeRecord]):
    ...
    for r in records:
        ...
        if not kol:
            continue
        direction = classify_fund(r.fund_name, r.opinion_text)[0]
...
def _direction_signal(
    ...
    「其他/待分类」方向不产生强趋势信号（明确减仓/共识升温等），
    因为该方向可能混合了多种不同主题的基金。
    """
    # 「其他/待分类」只展示基础数据，不生成强信号
...
def build(analysis_date, today_records: List[TradeRecord], today_complete: bool = True) -> dict:
    ...
    for direction in sorted(all_dirs):
        ...
        signal_type = _direction_signal(direction, today_kol_count, today_sell_kol_count, buy_change_pct)
...
def _direction_today_buy(records: List[TradeRecord], kol: str, direction: str) -> float:
    ...
    for r in records:
        if (r.kol_name or "").strip() != kol:
            continue
        if classify_fund(r.fund_name, r.opinion_text)[0] != direction:
            continue
        if _is_buy(r.operation_type):
